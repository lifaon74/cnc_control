...
export class Timer {
  ...
  static getBestUnit(value: number): string {
    let unit: string;
    for(unit in this.units) {
      if((value / this.units[unit]) > 1) {
        break;
      }
    }
    return unit;
  ...
  static toString(value: number, valueUnit: string, outUnit: string = this.getBestUnit(value)): string {
    return String(Math.round(Timer.convert(value, valueUnit, outUnit) * 100) / 100) + outUnit;
  }
  ...
  disp(message?: string, unit?: string): this {
    let diff = this.diff();
    message = message || '';
    console.log(message + ' ' + Timer.toString(diff, 'ns', unit));
    return this;
  };
